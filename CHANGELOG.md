...
- New operation `hs_register_source` (`POST /v1/register-source`): register a source the workspace
  ...
  as the ingest completes), `warehouse` or `event_stream` (registered by contract). Free; a
  `hsk_test_` key answers 403 `test_key_sample_only`. Reached from the Python client
  (`Client.register_source`), both framework adapters and the n8n node (`Register a source`).
- `openapi/openapi.json` is now generated by the Datagoat product repository's `packages/mcp`
  (`pnpm mcp:openapi`); `x-hs-deprecation.served_in_parallel` gains `2.0.0`.
